module Compute
  ...
  def self.give_hash(rental:, car:)
    hash_to_return = {
      "id" => rental.id,
      "price" => compute_price(rental: rental, car: car),
      "commission" => body_commission(
        total_fee: total_fee(compute_price(rental: rental, car: car)),
        days: rental.days
      )
    }
  end

  module_function def body_commission(total_fee:, days:)
    {
      "insurance_fee": insurance_fee(total_fee),
      "assistance_fee": assistance_fee(days),
      "drivy_fee": drivy_fee(fee: total_fee, insurance_fee: insurance_fee(total_fee), assistance_fee: assistance_fee(days))
    }
  end
